...
func generateScene():
	...
	for x in range(0, 10):
		...
		var proposed_coord = position+(direction_offset_cor+direction_offset_room)
		var proposed_coord_abs = proposed_coord + direction_offset_cor + direction_offset_room
		var proposed_placement = AABB(proposed_coord_abs,corridorlist[0].get_aabb().size)
